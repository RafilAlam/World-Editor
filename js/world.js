class World {
    ...
    generateCorridor(start, end) {
        ...
        this.graph.removePoint(projStart, false);
        this.graph.removePoint(projEnd, false);
        ...
        for (let i = 0; i < bases.length - 1; i++) {
            for (let j = i + 1; j < bases.length; j++) {
                if (
                    ...
                    Math.round(bases[i].distanceToPoly(bases[j])) < this.spacing
            ) {
                    bases.splice(j, 1);
                    j--;
                }
    ...
    #updateLights() {
        ...
        for (const light of lights) {
            ...
            if (!controlCenter && point) {
                ...
                this.markings.splice(this.markings.indexOf(light), 1)
            }
        }
    ...
    draw(ctx, viewPoint, showStartMarkings = true) {
        ...
        for (const seg of this.graph.segments) {
            seg.draw(ctx, { colour: "white", width: 4, dash: [10, 10] })
        }
        for (const seg of this.roadBorders) {
            seg.draw(ctx, { colour: "white", width: 4 });
        }
